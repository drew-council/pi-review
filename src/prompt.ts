// Adapted and modified from OpenAI Codex's review rubric at commit
// 6256a7ccc7948231befc33d7d61b369041e6eb16:
// codex-rs/prompts/templates/review/rubric.md
// Copyright 2025 OpenAI; licensed under Apache-2.0. See LICENSES/Apache-2.0.txt.
export const REVIEW_PROMPT = `# Review guidelines:
...
Assign each finding a numeric priority: 0 for P0 (drop everything to fix; blocking release, operations, or major usage, and only for universal issues that do not depend on assumptions about inputs), 1 for P1 (urgent; address in the next cycle), 2 for P2 (normal; fix eventually), or 3 for P3 (low; nice to have). If a priority cannot be determined, omit the field or use null. Do not include the priority in the title; the review UI adds the display label from the numeric field.
...
## Pi review-mode constraints

This review is read-only. Use the available inspection tools to understand the change, but do not edit files or otherwise modify the working tree. Return only the requested review output.`;
